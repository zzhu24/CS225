...
return filler::dfs::fill(img,x,y,temp,tolerance,frameFreq);
}
...
animation filler::bfs::fillSolid( PNG & img, int x, int y, 
     ...
     *  correct call to fill with the correct colorPicker parameter.
     */solidColorPicker temp = solidColorPicker(fillColor);
return filler::bfs::fill(img,x,y,temp,tolerance,frameFreq);
}
...
animation filler::fill( PNG & img, int x, int y,
    ...
    OrderingStructure<int> xaxis,yaxis;
    RGBAPixel ori = *img(x,y);
    animation result;
    int count = 0, width = img.width(), height = img.height();
...
processed[i] = new bool[height];
    ...
    yaxis.add(y);
    while(!yaxis.isEmpty()){
        int tempx = xaxis.remove(), tempy = yaxis.remove();
        bool isTolerant = tolerance >= pow((img(tempx,tempy)->red-ori.red),2)+pow((img(tempx,tempy)->green-ori.green),2)+pow((img(tempx,tempy)->blue-ori.blue),2);
        bool enterif = ((!processed[tempx][tempy])  &&  isTolerant);
...
bool  ri= (count==frameFreq);
            ...
            if((tempx+1)<width&&!processed[tempx+1][tempy]){
                xaxis.add(tempx+1);
                yaxis.add(tempy);
            }
            if((tempy+1)<height && !processed[tempx][tempy+1]){
                xaxis.add(tempx);
                yaxis.add(tempy+1);
            }
            if((tempx-1)>=0 && !processed[tempx-1][tempy]){
                xaxis.add(tempx-1);
                yaxis.add(tempy);
            }
            if((tempy-1)>=0 && !processed[tempx][tempy-1]){
                xaxis.add(tempx);
                yaxis.add(tempy-1);
            }
        }
