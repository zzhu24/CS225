...
class BinaryTree
{
	...
	/**
	 *helper function of mirror function
	*/
	void mirror(Node * subRoot);
        
	/**
	*helper funtion of isOrdered
	*/
	bool isOrdered(const Node * subRoot) const;

	/**
	*helper function of sumDistance
	*/
	int sumDistances(Node * subRoot , int sum) const;

	/**
	*helper function of PRINTPATH
	*/
	void printPaths(Node * subNode, vector<T> path) const;
