	...
	if(subRoot == NULL){
		...
		if(subRoot -> left != NULL){
			if(subRoot -> elem < subRoot -> left -> elem){
				return false;}
			if(root -> elem < subRoot -> left -> elem){
				return false;}}
		if(subRoot -> right != NULL){
			if(subRoot -> right -> elem < subRoot -> elem){
				return false;}
			if(subRoot -> right -> elem < root -> elem){
				return false;}
			if(subRoot -> left == NULL){
				if(subRoot -> elem < subRoot -> right -> elem){
					return true;}}}}
	order = (isOrdered(subRoot -> left) && isOrdered(subRoot -> right));
	return order;}
...
int BinaryTree<T>::sumDistances(Node * subRoot , int sum) const{
	if (subRoot == NULL){
		return 0;}
	return sum + sumDistances(subRoot->right, sum + 1) + sumDistances(subRoot->left, sum + 1);}
