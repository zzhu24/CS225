...
LogfileParser::LogfileParser( const string & fname ) : whenVisitedTable( 256 ) {
    ...
    while( infile.good() ) {
         ...
         * vector as well.
         */
string c_url = ll.customer+ll.url;
if(whenVisitedTable.keyExists(c_url)){
if(whenVisitedTable[c_url]<ll.date)
whenVisitedTable[c_url] = ll.date;
}
else whenVisitedTable.insert(c_url,ll.date);
...
time_t LogfileParser::dateVisited( const string & customer, const string & url ) const {
    ...
    string c_url = customer + url;
if(whenVisitedTable.keyExists(c_url))
return whenVisitedTable.find(c_url);
