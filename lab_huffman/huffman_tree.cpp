...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	...
	for(unsigned int i = 0; i < frequencies.size(); i ++){
		TreeNode * newSingle = new TreeNode(frequencies[i]);
		singleQueue.push(newSingle);}
	while(mergeQueue.size() + singleQueue.size() > 1){
		TreeNode * temp1 = removeSmallest(mergeQueue, singleQueue);
		TreeNode * temp2 = removeSmallest(mergeQueue, singleQueue);
		TreeNode * internal = new TreeNode(temp1 -> freq.getFrequency() + temp2 -> freq.getFrequency());
		internal -> left = temp1;
		internal -> right = temp2;
		mergeQueue.push(internal);}
	root = removeSmallest(mergeQueue, singleQueue);}
...
void HuffmanTree::writeTree( TreeNode * current, BinaryFileWriter & bfile ) {
	 ...
	 */
	//if it is not a leaf node it will not contian a character
	if(current->left == NULL &&current->right == NULL){
		bfile.writeBit(1);
...
HuffmanTree::TreeNode * HuffmanTree::readTree( BinaryFileReader & bfile ) {
	...
	while(bfile.hasBits()){
		if(bfile.getNextBit()==1){
			Frequency temp(bfile.getNextByte(),0);
			TreeNode * leaf = new TreeNode(temp);
			return leaf;}
		else{
			TreeNode * intnode = new TreeNode(0);
			intnode->left = readTree(bfile);
			intnode->right = readTree(bfile);
			return intnode;}}
	return NULL;}
